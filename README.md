# Korean-life-with-you-

<!DOCTYPE html>
<html lang="zh-CN">
<head>
<!-- Firebase CDN -->
<script type="module">
  // Import the functions you need from the SDKs you need
  import { initializeApp } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-app.js";
  import { getDatabase, ref, push, set } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-database.js";

  // 全局暴露给 window，React 才能用
  window.initializeApp = initializeApp;
  window.getDatabase = getDatabase;
  window.ref = ref;
  window.push = push;
  window.set = set;
</script>

  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>韩语对话练习 - 留学生场景模拟</title>
  <script crossorigin src="https://cdnjs.cloudflare.com/ajax/libs/react/18.2.0/umd/react.production.min.js"></script>
  <script crossorigin src="https://cdnjs.cloudflare.com/ajax/libs/react-dom/18.2.0/umd/react-dom.production.min.js"></script>
  <script src="https://cdnjs.cloudflare.com/ajax/libs/babel-standalone/7.23.5/babel.min.js"></script>
  <script src="https://cdn.tailwindcss.com"></script>
  <style>
    @keyframes bounce {
      0%, 100% { transform: translateY(0); }
      50% { transform: translateY(-10px); }
    }
    .animate-bounce {
      animation: bounce 1s infinite;
    }
  </style>
</head>
<body>
  <div id="root"></div>
  const firebaseConfig = {
  apiKey: "你的",
  authDomain: "你的",
  databaseURL: "你的",
  projectId: "你的",
  storageBucket: "你的",
  messagingSenderId: "你的",
  appId: "你的",
  measurementId: "你的"
};

// 初始化 Firebase
const app = window.initializeApp(firebaseConfig);
const db = window.getDatabase(app);

  <script type="text/babel">
    const { useState } = React;

    const Home = ({ size }) => (
      <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        <path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"></path>
        <polyline points="9 22 9 12 15 12 15 22"></polyline>
      </svg>
    );

    const RotateCcw = ({ size }) => (
      <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        <polyline points="1 4 1 10 7 10"></polyline>
        <path d="M3.51 15a9 9 0 1 0 2.13-9.36L1 10"></path>
      </svg>
    );

    const Send = ({ size }) => (
      <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        <line x1="22" y1="2" x2="11" y2="13"></line>
        <polygon points="22 2 15 22 11 13 2 9 22 2"></polygon>
      </svg>
    );

    const User = ({ size }) => (
      <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path>
        <circle cx="12" cy="7" r="4"></circle>
      </svg>
    );

    function KoreanConversationApp() {
      const [selectedScenario, setSelectedScenario] = useState(null);
      const [messages, setMessages] = useState([]);
      const [inputText, setInputText] = useState('');
      const [isLoading, setIsLoading] = useState(false);

      const scenarios = [
        {
          id: 'landlord',
          title: '房东',
          icon: '🏠',
          character: '김 사장님 (房东金先生)',
          greeting: '안녕하세요! 새로 오신 유학생이시죠? 집에 대해 설명해드릴게요.',
          description: '租房、维修、生活问题'
        },
        {
          id: 'professor',
          title: '教授',
          icon: '👨‍🏫',
          character: '이 교수님 (李教授)',
          greeting: '안녕하세요. 이번 학기 수업에 오신 걸 환영합니다. 질문 있으세요?',
          description: '课程咨询、作业问题'
        },
        {
          id: 'friend',
          title: '朋友',
          icon: '👥',
          character: '민지 (敏智)',
          greeting: '안녕! 너 중국에서 온 유학생이지? 우리 친구하자!',
          description: '日常聊天、约饭'
        },
        {
          id: 'bank',
          title: '银行',
          icon: '🏦',
          character: '은행 직원 (银行职员)',
          greeting: '안녕하세요, 고객님. 무엇을 도와드릴까요?',
          description: '开户、汇款、取款'
        },
        {
          id: 'bus',
          title: '公交车',
          icon: '🚌',
          character: '버스 기사님 (公交司机)',
          greeting: '어디까지 가세요?',
          description: '问路、购票'
        },
        {
          id: 'convenience',
          title: '便利店',
          icon: '🏪',
          character: '편의점 알바생 (便利店员工)',
          greeting: '어서오세요! 필요하신 거 있으세요?',
          description: '购物、付款'
        },
        {
          id: 'post',
          title: '邮局',
          icon: '📮',
          character: '우체국 직원 (邮局职员)',
          greeting: '안녕하세요. 무엇을 도와드릴까요?',
          description: '寄包裹、买邮票'
        },
        {
          id: 'restaurant',
          title: '餐厅',
          icon: '🍽️',
          character: '식당 직원 (餐厅服务员)',
          greeting: '어서오세요! 몇 분이세요?',
          description: '点餐、结账'
        }
      ];

      const getTranslation = (koreanText) => {
        const translations = {
          '안녕하세요! 새로 오신 유학생이시죠? 집에 대해 설명해드릴게요.': '你好！你是新来的留学生吧？我给你介绍一下房子。',
          '안녕하세요. 이번 학기 수업에 오신 걸 환영합니다. 질문 있으세요?': '你好。欢迎来到这学期的课程。有问题吗？',
          '안녕! 너 중국에서 온 유학생이지? 우리 친구하자!': '嗨！你是从中国来的留学生吧？我们做朋友吧！',
          '안녕하세요, 고객님. 무엇을 도와드릴까요?': '您好，顾客。需要什么帮助吗？',
          '어디까지 가세요?': '您要去哪里？',
          '어서오세요! 필요하신 거 있으세요?': '欢迎光临！需要什么吗？',
          '안녕하세요. 무엇을 도와드릴까요?': '您好。需要什么帮助吗？',
          '어서오세요! 몇 분이세요?': '欢迎光临！几位？'
        };
        return translations[koreanText] || '';
      };

      const startScenario = (scenario) => {
        setSelectedScenario(scenario);
        setMessages([
          {
            role: 'assistant',
            content: scenario.greeting,
            translation: getTranslation(scenario.greeting)
          }
        ]);
      };

      const sendMessage = async () => {
        if (!inputText.trim() || isLoading) return;

        const userMessage = {
          role: 'user',
          content: inputText
        };

        setMessages(prev => [...prev, userMessage]);
        setInputText('');
        setIsLoading(true);

        try {
          const response = await fetch('https://api.anthropic.com/v1/messages', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
            body: JSON.stringify({
              model: 'claude-sonnet-4-20250514',
              max_tokens: 1000,
              messages: [
                {
                  role: 'user',
                  content: '你是一个韩语对话教练。当前场景：' + selectedScenario.title + '（' + selectedScenario.character + '）。\n\n学生说了："' + inputText + '"\n\n请你做以下事情：\n1. 如果学生的韩语有语法错误、用词不当或不自然，请先指出错误并给出正确的说法\n2. 然后以' + selectedScenario.character + '的身份，用自然的韩语回应学生\n3. 在回应后提供中文翻译\n\n请按以下格式回复（如果没有错误就跳过纠错部分）：\n\n【纠错】\n❌ 错误：[学生的错误表达]\n✅ 正确：[正确的韩语表达]\n💡 说明：[简短解释]\n\n【回应】\n[韩语回应]\n\n【翻译】\n[中文翻译]\n\n保持对话自然、实用，符合' + selectedScenario.description + '的场景。'
                }
              ]
            })
          });

          const data = await response.json();
          const aiResponse = data.content.find(item => item.type === 'text')?.text || '죄송합니다. 다시 말씀해주세요.';

          setMessages(prev => [...prev, {
            role: 'assistant',
            content: aiResponse
          }]);
        } catch (error) {
          console.error('API Error:', error);
          setMessages(prev => [...prev, {
            role: 'assistant',
            content: '죄송합니다. 잠시 문제가 있습니다. 다시 시도해주세요.\n\n【翻译】\n抱歉，出现了问题。请重试。'
          }]);
        } finally {
          setIsLoading(false);
        }
      };

      const resetConversation = () => {
        setMessages([
          {
            role: 'assistant',
            content: selectedScenario.greeting,
            translation: getTranslation(selectedScenario.greeting)
          }
        ]);
        setInputText('');
      };

      const goBack = () => {
        setSelectedScenario(null);
        setMessages([]);
        setInputText('');
      };

      if (!selectedScenario) {
        return (
          <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-6">
            <div className="max-w-5xl mx-auto">
              <div className="text-center mb-12">
                <h1 className="text-4xl font-bold text-gray-800 mb-3">🇰🇷 韩语对话练习</h1>
                <p className="text-lg text-gray-600">留学生场景模拟 - AI实时纠错与对话</p>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {scenarios.map(scenario => (
                  <div
                    key={scenario.id}
                    onClick={() => startScenario(scenario)}
                    className="bg-white rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 cursor-pointer transform hover:-translate-y-2 p-6"
                  >
                    <div className="text-5xl mb-4 text-center">{scenario.icon}</div>
                    <h3 className="text-xl font-bold text-gray-800 mb-2 text-center">{scenario.title}</h3>
                    <p className="text-sm text-gray-600 mb-3 text-center">{scenario.character}</p>
                    <p className="text-xs text-gray-500 text-center">{scenario.description}</p>
                  </div>
                ))}
              </div>

              <div className="mt-12 bg-white rounded-xl shadow-lg p-6">
                <h2 className="text-2xl font-bold text-gray-800 mb-4">💡 使用说明</h2>
                <div className="space-y-3 text-gray-700">
                  <p>• 选择一个场景开始对话练习</p>
                  <p>• 用韩语输入你想说的话，AI会扮演场景中的角色回应你</p>
                  <p>• 如果你的韩语有错误，AI会先纠正，然后再回应</p>
                  <p>• 每个回应都会提供中文翻译帮助理解</p>
                  <p>• 点击"重新开始"可以重置当前场景的对话</p>
                </div>
              </div>
            </div>
          </div>
        );
      }

      return (
        <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex flex-col">
          <div className="bg-white shadow-md p-4">
            <div className="max-w-4xl mx-auto flex items-center justify-between">
              <button
                onClick={goBack}
                className="flex items-center gap-2 text-gray-600 hover:text-gray-800 transition-colors"
              >
                <Home size={20} />
                <span>返回场景选择</span>
              </button>
              <div className="flex items-center gap-3">
                <span className="text-3xl">{selectedScenario.icon}</span>
                <div>
                  <h2 className="text-xl font-bold text-gray-800">{selectedScenario.title}</h2>
                  <p className="text-sm text-gray-600">{selectedScenario.character}</p>
                </div>
              </div>
              <button
                onClick={resetConversation}
                className="flex items-center gap-2 bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 transition-colors"
              >
                <RotateCcw size={18} />
                <span>重新开始</span>
              </button>
            </div>
          </div>

          <div className="flex-1 overflow-y-auto p-6">
            <div className="max-w-4xl mx-auto space-y-4">
              {messages.map((msg, idx) => (
                <div
                  key={idx}
                  className={msg.role === 'user' ? 'flex justify-end' : 'flex justify-start'}
                >
                  <div
                    className={msg.role === 'user' ? 'max-w-2xl rounded-2xl p-4 bg-indigo-600 text-white' : 'max-w-2xl rounded-2xl p-4 bg-white text-gray-800 shadow-md'}
                  >
                    {msg.role === 'assistant' && (
                      <div className="flex items-center gap-2 mb-2">
                        <User size={18} />
                        <span className="font-semibold text-sm text-indigo-600">
                          {selectedScenario.character}
                        </span>
                      </div>
                    )}
                    <div className="whitespace-pre-wrap leading-relaxed">{msg.content}</div>
                    {msg.translation && (
                      <div className="mt-2 pt-2 border-t border-gray-200 text-sm text-gray-600">
                        {msg.translation}
                      </div>
                    )}
                  </div>
                </div>
              ))}
              {isLoading && (
                <div className="flex justify-start">
                  <div className="bg-white rounded-2xl p-4 shadow-md">
                    <div className="flex gap-2">
                      <div className="w-2 h-2 bg-indigo-600 rounded-full animate-bounce"></div>
                      <div className="w-2 h-2 bg-indigo-600 rounded-full animate-bounce" style={{animationDelay: '0.2s'}}></div>
                      <div className="w-2 h-2 bg-indigo-600 rounded-full animate-bounce" style={{animationDelay: '0.4s'}}></div>
                    </div>
                  </div>
                </div>
              )}
            </div>
          </div>

          <div className="bg-white border-t border-gray-200 p-4">
            <div className="max-w-4xl mx-auto flex gap-3">
              <input
                type="text"
                value={inputText}
                onChange={(e) => setInputText(e.target.value)}
                onKeyPress={(e) => e.key === 'Enter' && sendMessage()}
                placeholder="用韩语输入你想说的话..."
                className="flex-1 px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500"
                disabled={isLoading}
              />
              <button
                onClick={sendMessage}
                disabled={isLoading || !inputText.trim()}
                className="bg-indigo-600 text-white px-6 py-3 rounded-lg hover:bg-indigo-700 transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center gap-2"
              >
                <Send size={20} />
                <span>发送</span>
              </button>
            </div>
          </div>
        </div>
      );
    }

    ReactDOM.render(<KoreanConversationApp />, document.getElementById('root'));
  </script>
</body>
</html>
